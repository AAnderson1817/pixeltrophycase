...
| `npm test`                        | Playwright: behaviour tests + golden frames, against a Vite server on port 5179                                                                           |
...
| `npm run bench`                   | Frame-cost benchmark on `dist/index.html` (`-- --size 390x844 --dpr 3 --cpu 4`, `-- --url <dev server>`)                                                  |
| `npm run sprites`                 | Regenerate the armor atlas from `art/sheets/` (Python; `pip install -r tools/sprites/requirements.txt`)                                                   |
| `npm run format` / `format:check` | Prettier                                                                                                                                                  |

First run: `npm install && npx playwright install chromium`.

Before finishing any change: `npm test` must pass. If the change touches rendering, also run `npm run build && npm run bench` and check it stays inside the budget under Invariants.

## Module map (`src/`)

```
main.js            entry: errors handler first, styles, boot()
core/  errors.js   on-page error box        util.js     rnd/ri/lerp/clamp, mulberry32, MOTION, later()
...
card/  faces.js    back + front faces (64x90); FACE container
...
game/  state.js    S (all mutable game state), window.APP created here
       layout.js   integer scale + logical size, positions; LY container
       bag.js      collection, localStorage 'loot-pixel-armor-v1', grid, resetBag(); BAGS container
...
Phase machine (`S.phase`): `entering → idle → hitstop → revealed → (upgrading → revealed) → collecting → entering`.
...
- **No per-frame allocation in hot paths.** Pixel work goes through `Uint32Array` views over `ImageData`; reuse buffers (see `pfxBuf`, `SCN.VOID32`, rotation sheets). Cache anything that can be cached (glyphs, coins, chain links, halos).
...
- **Live bindings.** Modules share state through ES imports. A module may reassign only bindings it declares. A `let` that another module must reassign lives in that module's container object (`SCN`, `FACE`, `BAGS`, `LY`) or in `S`. Otherwise mutate in place (arrays/objects).
...
`PERF` (`core/perf.js`): `base` .62 is the fixed density cut; `dq` (.35–1) is moved by the governor in `game/loop.js` from script time per frame; `bloomOff`/`bloomSkip` are set by the compositor probe only if hiding `#bloom` restores the frame rate. `PACE.div` renders every Nth refresh on ≥90 Hz displays (120→60, 144→72, 240→60).

## Sprite pipeline

`tools/sprites/extract.py` cuts the 30 pieces from the six concept sheets in `art/sheets/`: flood-fill background removal, erase rects, rotation, premultiplied area downscale to ≤50×38 plus 16 px and 10 px bag icons, median-cut colour reduction, 1 px `k` outline. Writes `src/assets/armor-atlas.png`, `src/data/armor-atlas.json` and a preview at `tools/sprites/out/preview.png`. Per-piece crop boxes, rarity and names are the `PIECES` table at the top of the script. Output is deterministic; re-running on unchanged sheets must produce identical files.
